...
class ListItemSerializer(serializers.ModelSerializer):
    ...
    todo_item = serializers.PrimaryKeyRelatedField(queryset=TodoItem.objects.all(), required=False, allow_null=True, default=None)
...
class TodoItemSerializer(serializers.ModelSerializer):
    ...
    list_items = ListItemSerializer(many=True, required=False)
    ...
    def create(self, validated_data):
        ...
        #Else, raise an error.
        if "type" in validated_data and validated_data["type"] == 'L':
            if 'list_items' not in validated_data:
                raise serializers.ValidationError('List items are required for type L.')
        elif "type" in validated_data and validated_data["type"] == 'T':
            if 'text_value' not in validated_data:
                raise serializers.ValidationError('Field text_value is required for type T')
        elif "type" in validated_data and validated_data["type"] == 'U':
            if 'url_value' not in validated_data:
                raise serializers.ValidationError('Field url_value is required for type U')
        else:
